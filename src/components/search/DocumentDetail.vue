...
export default {
    ...
    computed: {
        ...
        effective_date(){
            ...
            return moment(effective_date, 'DD-MM-YYYY').isValid() ? moment(effective_date).format('DD-MM-YYYY') : "";
        },
        issued_date(){
            let issued_date = this.item["_source"]["attribute"]['issued_date'];
            return moment(issued_date, 'DD-MM-YYYY').isValid() ? moment(issued_date).format('DD-MM-YYYY') : "";
        }
    },
    methods :{
        handleClick(){
